...
func simple_encrypt(data: String, key = "My secret key!!!"):
...
func simple_decrypt(encrypted_data: PackedByteArray, key = "My secret key!!!"):
    
    var random_iv = encrypted_data.slice(0, 16)
    encrypted_data = encrypted_data.slice(16)
    
    var aes = AESContext.new()
    aes.start(AESContext.MODE_CBC_DECRYPT, key.to_ascii_buffer(), random_iv)
